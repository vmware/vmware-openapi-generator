...
def process_put_post_patch_request(url, service_name, operation_name, params,
    ...
    """
    ## Path
    path_param_list, other_param_list, new_url = api_open_ph.extract_path_parameters(params, url)
    par_array = []
    ...
    ## Body
    body_param_list, other_param_list =  api_open_ph.extract_body_parameters(other_param_list)
    ...
    ## Query 
    query_param_list, other_param_list = api_open_ph.extract_query_parameters(other_param_list)
...
def process_get_request(url, params, type_dict, structure_svc, enum_svc, enable_filtering):
    param_array = []
    path_param_list, other_params_list, new_url = api_open_ph.extract_path_parameters(params, url)
    ...
    query_param_list, other_params_list = api_open_ph.extract_query_parameters(other_params_list)
    #Query
    for query_param in query_param_list:
...
def process_delete_request(url, params, type_dict, structure_svc, enum_svc, enable_filtering):
    path_param_list, other_params, new_url = api_open_ph.extract_path_parameters(params, url)
    param_array = []
    for field_info in path_param_list:
        parx = api_open_ph.convert_field_info_to_swagger_parameter('path', field_info, type_dict,
                                                       structure_svc, enum_svc, enable_filtering)

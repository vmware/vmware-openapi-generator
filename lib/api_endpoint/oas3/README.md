...
```python
 def get_path(self, operation_info, http_method, url, service_name, type_dict, structure_dict, enum_dict,
    ...
    return path
```
From here onwards ```get_path()``` makes three important function calls to construct the openapi specification files.
 ...
 ```python
    def process_put_post_patch_request(self, url, service_name, operation_name, params,
        ...
        for query_param in query_param_list:
            parx = object.convert_field_info_to_swagger_parameter('query', query_param, type_dict,
                                                            structure_svc, enum_svc, enable_filtering)
            par_array.append(parx)
...
```python
    def process_get_request(self, url, params, type_dict, structure_svc, enum_svc, enable_filtering, object):
        ...
        #Query
        for query_param in query_param_list:
            parameter_obj = object.convert_field_info_to_swagger_parameter('query', query_param,
                                                                type_dict, structure_svc, enum_svc, enable_filtering)
            param_array.append(parameter_obj)
...
```
```process_get_request()``` also extracts the path and non-path parameters first. It deals with the field info of path parameters in the same manner as above. It extracts the query and non-query parameters from non-path parameters using ```extract_query_params()``` and the calls ```convert_field_info_to_swagger_parameter()``` to process field info of query parameters. ```flatten_query_param_spec()``` to take care of the non-query parameters as in the http methods of put, post and patch. The function creates a query parameter for every field info specification. If the field info is simple type i.e string or integer, then it is converted to swagger parameter. 
...
```populate_response_map()``` uses the operation info's output object to extract the output object type. It takes care of two types of responses one which lead to a success and others which throw an error in order to populate the ```response_map{}```. A ```success_response``` schema is declared and the schema of the success response by finding the output object's type using the apiTypeHandler class. If the schema is not void, response object is initialized with this schema along with storing this response object in ```type_dict{}```. For all the request codes that result in an 'ok' message, the success response is added as their value in the ```response_map{}```.

A failed response will be generated if there occurs an error in the execution of an operation. All the errors that can arise while executing an operation are listed in errors object of operation info. We iterate over these errors and get their corresponding response ```status_codes``` from ```error_map{}```. The type of the error is determined using check_type() of the apiTypeHandler class along with creating a ```response_object``` for that error. The ```response_map{}``` for these errors contains its key as the ```status_code``` of the error and value as a ```response_object``` having the documentation and schema for the error.

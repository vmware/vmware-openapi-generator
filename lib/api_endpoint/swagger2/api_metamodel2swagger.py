from lib.api_endpoint.swagger2 import api_swagger_parameter_handler as api_swagg_ph
from lib.api_endpoint.swagger2 import api_swagger_response_handler as api_swagg_rh
TAG_SEPARATOR = '/'
...
def process_put_post_patch_request(url, service_name, operation_name, params,
    ...
    """
    ## Path
    path_param_list, other_param_list, new_url = api_swagg_ph.extract_path_parameters(params, url)
    par_array = []
    for field_info in path_param_list:
        parx = api_swagg_ph.convert_field_info_to_swagger_parameter('path', field_info, type_dict,
                                                       structure_svc, enum_svc, enable_filtering)
        par_array.append(parx)

    ## Body
    body_param_list, other_param_list =  api_swagg_ph.extract_body_parameters(other_param_list)
    
    if body_param_list:
        parx = api_swagg_ph.wrap_body_params(service_name, operation_name, body_param_list, type_dict,
                                structure_svc, enum_svc, enable_filtering)
        if parx is not None:
            par_array.append(parx)
            
    ## Query 
    query_param_list, other_param_list = api_swagg_ph.extract_query_parameters(other_param_list)
    for query_param in query_param_list:
        parx = api_swagg_ph.convert_field_info_to_swagger_parameter('query', query_param, type_dict,
                                                        structure_svc, enum_svc, enable_filtering)
        par_array.append(parx)
    ...
    for field_info in other_param_list:
        ...
        flattened_params = api_swagg_ph.flatten_query_param_spec(field_info, type_dict, structure_svc, enum_svc, enable_filtering)
...
def process_get_request(url, params, type_dict, structure_svc, enum_svc, enable_filtering):
    param_array = []
    path_param_list, other_params_list, new_url = api_swagg_ph.extract_path_parameters(params, url)
    
    for field_info in path_param_list:
        parameter_obj = api_swagg_ph.convert_field_info_to_swagger_parameter('path', field_info,
                                                                type_dict, structure_svc, enum_svc, enable_filtering)
        param_array.append(parameter_obj)

    ## Query 
    query_param_list, other_params_list = api_swagg_ph.extract_query_parameters(other_params_list)
    for query_param in query_param_list:
        parameter_obj = api_swagg_ph.convert_field_info_to_swagger_parameter('query', query_param,
                                                            type_dict, structure_svc, enum_svc, enable_filtering)
        param_array.append(parameter_obj)
...
def process_delete_request(url, params, type_dict, structure_svc, enum_svc, enable_filtering):
    path_param_list, other_params, new_url = api_swagg_ph.extract_path_parameters(params, url)
    param_array = []
    for field_info in path_param_list:
        parx = api_swagg_ph.convert_field_info_to_swagger_parameter('path', field_info, type_dict,
                                                       structure_svc, enum_svc, enable_filtering)
    ...
    for field_info in other_params:
        parx = api_swagg_ph.convert_field_info_to_swagger_parameter('query', field_info, type_dict,
                                                        structure_svc, enum_svc, enable_filtering)
        param_array.append(parx)
    return param_array, new_url

from lib.rest_endpoint.swagger2 import rest_swagger_parameter_handler as rest_swagg_ph 
from lib.rest_endpoint.swagger2 import rest_swagger_response_handler as rest_swagg_rh
TAG_SEPARATOR = '/'
...
def process_put_post_patch_request(url, service_name, operation_name, params,
    ...
    """
    ## Path
    path_param_list, other_param_list, new_url = rest_swagg_ph.extract_path_parameters(params, url)
    par_array = []
    for field_info in path_param_list:
        parx = rest_swagg_ph.convert_field_info_to_swagger_parameter('path', field_info, type_dict,
                                                       structure_svc, enum_svc, enable_filtering)
    ...
    if body_param_list:
        parx = rest_swagg_ph.wrap_body_params(service_name, operation_name, body_param_list, type_dict,
                                structure_svc, enum_svc, enable_filtering)
        if parx is not None:
            par_array.append(parx)
...
def process_get_request(url, params, type_dict, structure_svc, enum_svc, enable_filtering):
    param_array = []
    path_param_list, other_params_list, new_url = rest_swagg_ph.extract_path_parameters(params, url)
    
    for field_info in path_param_list:
        parameter_obj = rest_swagg_ph.convert_field_info_to_swagger_parameter('path', field_info,
                                                                type_dict, structure_svc, enum_svc, enable_filtering)
    ...
    for field_info in other_params_list:
        ...
        flattened_params = rest_swagg_ph.flatten_query_param_spec(field_info, type_dict, structure_svc, enum_svc, enable_filtering)
...
def process_delete_request(url, params, type_dict, structure_svc, enum_svc, enable_filtering):
    path_param_list, other_params, new_url = rest_swagg_ph.extract_path_parameters(params, url)
    param_array = []
    for field_info in path_param_list:
        parx = rest_swagg_ph.convert_field_info_to_swagger_parameter('path', field_info, type_dict,
                                                       structure_svc, enum_svc, enable_filtering)
    ...
    for field_info in other_params:
        parx = rest_swagg_ph.convert_field_info_to_swagger_parameter('query', field_info, type_dict,
                                                        structure_svc, enum_svc, enable_filtering)
        param_array.append(parx)
    return param_array, new_url
